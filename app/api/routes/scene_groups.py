...
def list_scene_groups(project_id: int | None = None, episode_id: int | None = None, db: Session = Depends(get_db)) -> list[SceneGroup]:
...
def create_scene_group(payload: SceneGroupCreate, db: Session = Depends(get_db)) -> SceneGroup:
    group = SceneGroup(
...
def get_scene_group(group_id: int, db: Session = Depends(get_db)) -> SceneGroup:
...
def update_scene_group(group_id: int, payload: SceneGroupCreate, db: Session = Depends(get_db)) -> SceneGroup:
    group = db.get(SceneGroup, group_id)
    if not group:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="SceneGroup not found")
...
def delete_scene_group(group_id: int, db: Session = Depends(get_db)) -> None:
    group = db.get(SceneGroup, group_id)
    if not group:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="SceneGroup not found")
    db.delete(group)
